...
bool VerticalBlurShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"Shader/verticalblur.vs", L"Shader/verticalblur.ps");
	if(!result){
		return false;
	}
...
bool VerticalBlurShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenHeight)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, texture, screenHeight);
	if(!result)
	{
		return false;
	}
...
bool VerticalBlurShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* texture, float screenHeight)
{
	HRESULT result;
    D3D11_MAPPED_SUBRESOURCE mappedResource;
	ScreenSizeBufferType* dataPtr;
	...
	dataPtr = (ScreenSizeBufferType*)mappedResource.pData;

	// Copy the data into the constant buffer.
	dataPtr->screenHeight = screenHeight;
	dataPtr->padding = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
...
bool VerticalBlurShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, 
	D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenHeight, float screenWidth)
{
	return Render(deviceContext, indexCount, projectionMatrix, texture, screenHeight/2);
}
