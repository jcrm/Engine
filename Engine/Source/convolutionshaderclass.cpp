									 ...
									 D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, float screenHeight, float screenWidth)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, screenHeight, screenWidth);
	if(!result){
		return false;
	}
...
bool ConvolutionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC convolutionBufferDesc;
	...
	convolutionBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	convolutionBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	convolutionBufferDesc.MiscFlags = 0;
	convolutionBufferDesc.StructureByteStride = 0;
	...
	ID3D11ShaderResourceView* texture, float screenHeight, float screenWidth)
{
	...
	// Copy the data into the constant buffer.
	dataPtr3->weight0 = 0.25;
	dataPtr3->weight1 = 0.25;
	dataPtr3->weight2 = 0.25;

	dataPtr3->weight3 = 0.25;
	dataPtr3->weight4 = 0.25;
	dataPtr3->weight5 = 0.25;

	dataPtr3->weight6 = 0.25;
	dataPtr3->weight7 = 0.25;
	dataPtr3->weight8 = 0.25;
